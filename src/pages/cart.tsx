            ...
            <ul>
                {storeContext.cart.lineItems.map((lineItem) => (
                    <li key={lineItem.id}>
                        ...
                        <button onClick={() => doIncrement(lineItem.id as string, lineItem.quantity)}>増やす</button>
                        <button onClick={() => doDecrement(lineItem.id as string, lineItem.quantity)}>減らす</button>
                        <button onClick={() => handleRemove(lineItem.id as string)}>削除</button>
                    </li>
                ))}
            </ul>
        </>
